...
def running_segment(time, flux, window_length, edge_cutoff, cval, ftol):
    ...
    lo = numpy.min(time) + edge_cutoff
    hi = numpy.max(time) - edge_cutoff
    idx_start = 0
    idx_end = 0

    for i in range(size-1):
        if time[i] > lo and time[i] < hi:
            # Nice style would be:
...
def flatten(time, flux, window_length, edge_cutoff=0, break_tolerance=None, cval=6, 
    ...
    for i in range(len(gaps_indexes)-1):
        ...
        trend_segment = running_segment(
            time_compressed[lo:hi],
            flux_compressed[lo:hi],
            window_length,
            edge_cutoff,
            cval,
            ftol)
    ...
    full_trend_flux = numpy.full(len(time), numpy.nan)
    mask = numpy.where(mask==False)[0]
    for idx in range(len(mask)):
        full_trend_flux[mask[idx]] = trend_flux[idx]

    flatten_flux = flux / full_trend_flux
    if return_trend:
        return flatten_flux, full_trend_flux
    else:
        return flatten_flux
